...
interface VideoCardProps {
  ...
  isPlaying: boolean;
  accentColor?: "accent-orange" | "accent-pink" | "accent-blue" | "accent-red" | "primary";
}
...
export function VideoCard({ 
  ...
  return (
    ...
    >
      <div className={`rounded-xl p-0.5 relative ${isPlaying ? `border ${accentColor === 'primary' ? 'border-primary' : `border-${accentColor}`}` : ''}`}>
        <div className={`relative rounded-xl h-full overflow-hidden border ${
          isPlaying 
            ? accentColor === 'primary' 
              ? 'border-primary/50' 
              : `border-${accentColor}`
          ...
          <div className="flex items-center gap-4 p-3">
            ...
            <div className="relative w-20 h-20 overflow-hidden rounded-lg flex-shrink-0">
              ...
              {/* Simple overlay */}
              <div className={`absolute inset-0 bg-black/50 ${isPlaying ? 'opacity-30' : 'opacity-60'} transition-opacity duration-300`}></div>
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                {isPlaying ? (
                  <Pause className={`h-8 w-8 ${accentColor === 'primary' ? 'text-primary' : `text-${accentColor}`}`} />
                ) : (
